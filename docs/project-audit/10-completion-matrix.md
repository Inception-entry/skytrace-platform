...
- [x] Admin 头像按 magic-byte 入库（AS-09）。AI 图片/视频有界读入与 FFmpeg 超时（RB-14）。AI 解码前像素预算（AI-04）。Java/Node 证据、知识库、视觉上传按 magic-byte（RB-12）。知识库 PDF 页数/字数/切片/超时（AI-01）。未改 RAG、多层内存复制、可终止解析进程池。
