...
- [x] Admin 头像按 magic-byte 入库（AS-09）。AI 图片/视频有界读入与 FFmpeg 超时（RB-14）。AI 解码前像素预算（AI-04）。Java/Node 证据、知识库、视觉上传按 magic-byte（RB-12）。知识库 PDF 页数/字数/切片/超时（AI-01）。Java 知识库/视觉流转发、Node 去掉额外 Uint8Array（BN-04）。PDF 解析超时会杀掉子进程（AI-01）。Node BFF 证据/知识库/视觉 multer 落盘（BN-04）。未改 RAG、cgroup 内存上限、Admin 头像落盘。
