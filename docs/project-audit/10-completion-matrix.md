...
- [x] Flyway V20 补 `inspection_task`（JV-01）。detection 可选 `detectionId` + `source_detection_id` 唯一键（JV-03）。detection 队列 DLQ + 有限重试后拒绝（JV-03）。未做 outbox、publisher confirm。
