...
- [x] Admin 前端无 refresh token 的 401 不再永久挂起（RB-08）。登出带着捕获 token 撤销服务端 refresh（RB-09）。login+`/me` 半登录回滚（FE-05）。refresh 带随机 `jti` 且并发消费只成功一次（RB-10）。JWT secret 启动 fail-fast（RB-11）。登录/刷新进程内限流（AS-06）。family 复用撤销、Redis 跨副本限流未做。没有修 Cesium、轮询、SSE 或 Socket。
